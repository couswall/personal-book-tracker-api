...
export class SearchBookDto {
    ...
    static create(object: ISearchBookDto | undefined): [string?, SearchBookDto?] {
        ...
        const {printType} = object;
        let {searchText, page, maxResults} = object;

        const [searchTextError, trimmedSearchText = ''] = isValidString(
            'searchText',
            searchText,
            1,
            50,
        ...
        const [maxResultError, parsedMaxResult] = isValidRequiredNumber(
            'maxResults',
            maxResults,
            true
        );
