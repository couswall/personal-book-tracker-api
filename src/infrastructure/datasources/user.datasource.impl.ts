...
export class UserDatasourceImpl implements UserDatasource {
    async create(createUserDto: CreateUserDto): Promise<UserEntity> {
        ...
        if (existingUsername)
            throw CustomError.badRequest(ERROR_MESSAGES.USER.CREATE.EXISTING_USERNAME);
        if (existingEmail)
            throw CustomError.badRequest(ERROR_MESSAGES.USER.CREATE.EXISTING_EMAIL);
    ...
    async login(loginUserDto: LoginUserDto): Promise<UserEntity> {
        ...
        if (!user)
            throw CustomError.badRequest(ERROR_MESSAGES.USER.LOGIN.INVALID_CREDENTIALS);
    ...
    async getById(getUserByIdDto: GetUserByIdDto): Promise<UserEntity> {
        ...
        if (!user)
            throw CustomError.badRequest(ERROR_MESSAGES.USER.GET_BY_ID.NO_EXISTING);
